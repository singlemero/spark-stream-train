...
/**
  ...
  * jackson和json4s
  */
object JacksonExample {
  def main(args: Array[String]): Unit = {
    //print(new BigDecimal(new java.math.BigDecimal(0)))
    val z = ZZ(BigInt(3), BigDecimal(33),"adfadsf")
    val gson = new Gson()
    //printf(gson.toJson(z))
    ...
    //val g = parse(str).extract[List[VoucherItems]]
    //jackson
    ...
    val person2 = mapper.readValue(str, classOf[List[VoucherItems]])
    println(person2)
//    implicit val formats = Serialization.formats(NoTypeHints)
    val ll = sread[List[VoucherItems]](str)
    printf("")
  }
}
