...
// 测试
  ...
  /**
    ...
    * @param sparkSession
    */
  def before(dataFrame: DataFrame, infoDF: DataFrame, sparkSession: SparkSession)(implicit briefMeta: BriefMeta) = {
...
//      rd.rdd.sortBy(_.accountPeriod).p
      rd.foreachPartition(p => {
        ...
        Try(insetBalanceInitial(c, briefMeta)) match {
          case Failure(ex) => {
  ...
  /**
    ...
    * @param sparkSession
    */
  def after(dataFrame: DataFrame, balanceDF: DataFrame, sparkSession: SparkSession)(implicit briefMeta: BriefMeta) = {
    import com.spsoft.spark.hint.DataFrameHints._
    import com.spsoft.spark.hint.DateHints._
    import sparkSession.implicits._
//    println(dataFrame.rdd.partitions.size)
  ...
  /**
    ...
    * @param sparkSession
    */
  def updateR (dstreamDF: DataFrame, balanceDF: DataFrame, sparkSession: SparkSession)(implicit briefMeta: BriefMeta) = {
...
//        val pid = TaskContext.get().partitionId()
        ...
        if(!ls.isEmpty){
          Try(updateBalance(ls,briefMeta)) match {
            case Failure(ex) => {
...
*/
  ...
  /**
    ...
    * @return
    */
  def updateBalance(data: Iterator[Array[Any]], briefMeta: BriefMeta): Unit ={
    if(!data.isEmpty){
      val update =
...
//      TODO
  ...
  /**
    ...
    * @return
    */
  def insetBalanceInitial(data: Iterator[Array[Any]], briefMeta: BriefMeta) = {
    if(!data.isEmpty){
